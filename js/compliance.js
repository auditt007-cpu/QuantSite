(function (root) {
  const RISK =
    // [PLAIN-TAG]
    "風險提示：歷史試算資料僅供策略研究與分析參考，不構成任何投資建議。加密貨幣市場具備高波動風險，請審慎評估。";

  const LEGAL = {
    risk:
      // [PLAIN-TAG]
      "本站提供之策略、信號、歷史試算與真錢交易展示內容僅供策略研究與教育用途。虛擬資產價格波動劇烈，可能導致本金重大損失。請依自身風險承受能力獨立決策，並諮詢合格顧問。",
    tos:
    ...
    disclaimer:
      // [PLAIN-TAG]
      "歷史績效不代表未來結果。歷史試算未完整計入滑點、手續費、資金費率與流動性衝擊。任何依本站資訊採取之交易行為，風險與損益由您自行承擔。",
  };
